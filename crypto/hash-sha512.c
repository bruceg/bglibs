 ...
 * Copyright (C) 2003,2005,2006  Bruce Guenter <bruce@untroubled.org>
 *
...
const struct hash_state sha512_init = {
  {
...
void sha512_compress(struct hash_state* state, const unsigned char* block)
{
  uint64 a = state->H[0];
  uint64 b = state->H[1];
  uint64 c = state->H[2];
  ...
  for (Wp = W, j = 0; j < 16; ++j, ++Wp, block += 8)
    *Wp = uint64_get_msb(block);
...
void sha512_extract(const struct hash_state* state, unsigned char* digest)
{
  unsigned i;
  for (i = 0; i < 8; ++i)
    uint64_pack_msb(state->H[i], digest + i*8);
}

const struct hash_control_block hash_sha512 = {
...
static void test(const char* s)
{
  ...
  hash_block(&hash_sha512, s, strlen(s), digest);
