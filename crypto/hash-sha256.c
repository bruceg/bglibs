...
/* hash-sha256.c - SHA-256 algorithm
...
const struct hash_state sha256_init = {
  {
...
void sha256_compress(struct hash_state* state, const unsigned char* block)
{
  const uint8* M = block;
  unsigned j;
  uint32 a = state->H[0];
...
void sha256_extract(const struct hash_state* state, unsigned char* digest)
{
  unsigned i;
  for (i = 0; i < 8; ++i)
    uint32_pack_msb(state->H[i], digest + i*4);
}

const struct hash_control_block hash_sha256 = {
...
static void test(const char* s)
{
  ...
  hash_block(&hash_sha256, s, strlen(s), digest);
